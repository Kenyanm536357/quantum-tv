...
import {
  Clapperboard, ChevronLeft, ChevronRight, Search, X, Play, Loader2,
  Grid3X3, List, Tv2
} from 'lucide-react';
...
function SeriesCard({ stream, onPlay }) {
    ...
    <section className="mb-8">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1.5">
          {onViewAll && <button onClick={onViewAll} className="text-[11px] font-bold text-pink-400 hover:text-pink-300 mr-2 transition-colors">See all →</button>}
      ...
      <div ref={ref} className="flex gap-3 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none' }}>
        {streams.slice(0, 40).map(s => (
          <div key={s.stream_id || s.url} style={{ width: 190, flexShrink: 0 }}>
            <SeriesCard stream={s} onPlay={onPlay} />
          </div>
        ))}
      </div>
    </section>
...
export default function SeriesSection() {
  ...
  const seriesCats = useMemo(() => {
    if (!playlist) return [];
    const keywords = ['series', 'show', 'episode', 'season', 'tv show', 'sitcom', 'documentary', 'reality'];
  ...
  const globalSearch = useMemo(() => {
    ...
    return seriesCats.flatMap(c => catStreamMap[c.category_id] || [])
      .filter(s => cleanName(s.name).toLowerCase().includes(q)).slice(0, 60);
  }, [search, selectedCat, seriesCats, catStreamMap]);

  if (loading) return (
    <div className="h-full flex flex-col items-center justify-center gap-4">
      <Loader2 className="w-8 h-8 text-pink-400 animate-spin" />
      <p className="text-sm text-white/40">Loading series…</p>
    </div>
  );

  if (error) return (
    <div className="h-full flex flex-col items-center justify-center gap-4 px-6 text-center">
      <Clapperboard className="w-10 h-10 text-white/15" />
      <p className="text-sm text-white/40">{error}</p>
      <button onClick={refresh} className="px-5 py-2.5 rounded-xl bg-pink-500/15 border border-pink-500/30 text-pink-400 text-sm font-semibold hover:bg-pink-500/25 transition-colors">Try Again</button>
    </div>
  );
          ...
          <div className="ml-auto flex items-center gap-2">
            <div className="relative">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search…"
                className="bg-white/5 border border-white/8 rounded-xl pl-9 pr-8 py-2 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-pink-500/40 transition-all w-44" />
            ...
            <div className="flex items-center bg-white/5 border border-white/8 rounded-xl p-0.5">
              <button onClick={() => setViewMode('grid')} className={`p-1.5 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-pink-500/20 text-pink-400' : 'text-white/35 hover:text-white'}`}><Grid3X3 className="w-3.5 h-3.5" /></button>
              <button onClick={() => setViewMode('list')} className={`p-1.5 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-pink-500/20 text-pink-400' : 'text-white/35 hover:text-white'}`}><List className="w-3.5 h-3.5" /></button>
            </div>
          </div>
        </div>
        <div className="flex-1 overflow-y-auto min-h-0">
          {viewMode === 'grid' ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
              {displayedItems.map(s => <SeriesCard key={s.stream_id || s.url} stream={s} onPlay={playM3UStream} />)}
            </div>
          ) : (
            <div className="space-y-1">
              {displayedItems.map((s, i) => (
                <div key={s.stream_id || s.url} onClick={() => playM3UStream(s)}
                  ...
                  <div className="w-14 h-8 rounded overflow-hidden flex-shrink-0" style={{ background: `linear-gradient(135deg, ${gradientFor(cleanName(s.name)).join(',')})` }}>
                  ...
                  <Play className="w-3.5 h-3.5 text-white/20 group-hover:text-pink-400 transition-colors flex-shrink-0" />
                </div>
              ))}
            </div>
          )}
      ...
      <div className="flex items-center gap-3 mb-6 flex-shrink-0 flex-wrap">
        <div>
          <h2 className="text-xl font-black text-white flex items-center gap-2">
            <Clapperboard className="w-5 h-5 text-pink-400" /> TV Shows & Series
          </h2>
          <p className="text-xs text-white/30 mt-0.5">{seriesCats.length} categories</p>
        </div>
        <div className="ml-auto relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white/25 pointer-events-none" />
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search all series…"
            className="bg-white/5 border border-white/8 rounded-xl pl-9 pr-8 py-2 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-pink-500/40 transition-all w-48" />
          {search && <button onClick={() => setSearch('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-white/30 hover:text-white"><X className="w-3.5 h-3.5" /></button>}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {search ? (
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
              {globalSearch.map(s => <SeriesCard key={s.stream_id || s.url} stream={s} onPlay={playM3UStream} />)}
            </div>
            {globalSearch.length === 0 && <p className="text-center text-white/25 py-16 text-sm">No series found.</p>}
          </div>
        ) : (
          <>
            {seriesCats.map(cat => (
            ...
            {seriesCats.length === 0 && <p className="text-center text-white/25 py-16 text-sm">No series available.</p>}
          </>
        )}
      </div>
    </div>
